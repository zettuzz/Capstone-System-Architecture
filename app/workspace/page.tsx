...
function WorkspaceContent() {
  ...
  useEffect(() => {
    if (isLoading) return;
    const completedIdea = nodes.find((n: any) => {
      const data = n.data as WorkspaceNodeData;
      return data.nodeType === 'idea' && data.phase === 'complete';
    });
    if (!completedIdea) return;
    if (autoGenerated.has(completedIdea.id)) return;
    if (blueprintData[completedIdea.id]) return;

    const hasUserKey = !!userKeys[provider];
    if (!hasUserKey && !canMakeRequest()) return;

    setAutoGenerated(prev => new Set(prev).add(completedIdea.id));
    setShowBlueprint(true);
    setActiveBlueprintRoot(completedIdea.id);
    setBlueprintLoadingRoot(completedIdea.id);

    const generate = async () => {
      try {
        const subtree = getSubtreeNodes(completedIdea.id, nodesRef.current, edgesRef.current);
        ...
        setBlueprintData((prev) => ({
          ...prev,
          [completedIdea.id]: { markdown: exportData.markdown, schedule: exportData.schedule || [] },
        }));
      } catch (err) {
        console.error('Auto-blueprint generation failed:', err);
        setBlueprintData((prev) => ({
          ...prev,
          [completedIdea.id]: { markdown: '# Blueprint Generation Failed\n\nPlease try again.', schedule: [] },
        }));
      } finally {
    ...
    generate();
  }, [nodes, isLoading, autoGenerated, blueprintData, provider, userKeys, canMakeRequest, projectTitle, nodesRef]);
  ...
  const hasBlueprint = Object.keys(blueprintData).length > 0;

  const handleShowBlueprint = useCallback(() => {
    const firstIdea = nodes.find((n: any) => {
      const data = n.data as WorkspaceNodeData;
      return data.nodeType === 'idea' && blueprintData[n.id];
    });
    if (firstIdea) {
      setActiveBlueprintRoot(firstIdea.id);
      setShowBlueprint(true);
    }
  }, [nodes, blueprintData]);
