...
export default function Page() {
  ...
  return (
    <div className="min-h-screen flex antialiased selection:bg-white selection:text-black">
      {isLoaded && isSignedIn && <Sidebar />}
      
      <div className={`flex-1 flex flex-col justify-center items-center relative ${isLoaded && isSignedIn ? '' : 'min-h-screen'}`}>
        {isLoaded && !isSignedIn && (
        ...
        <main className="w-full max-w-[960px] px-6 flex flex-col items-center justify-center z-10">
          ...
          <div className="w-full max-w-[640px] relative group">
            <div className="sharp-panel sharp-input rounded-none flex items-end w-full px-4 py-3 transition-all duration-200">
              <span aria-hidden="true" className="material-symbols-outlined text-text-muted mr-3 select-none mb-0.5" style={{ fontVariationSettings: "'FILL' 0 'wght' 400 'GRAD' 0 'opsz' 24" }}>
                terminal
              </span>
              <textarea
                ref={textareaRef}
                ...
                className="flex-1 bg-transparent border-none text-text-main text-[14px] placeholder:text-text-muted focus:ring-0 p-0 font-mono outline-none resize-none overflow-hidden leading-tight"
